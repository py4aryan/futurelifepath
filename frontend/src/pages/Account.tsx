...
import {
  ...
  CreditCard, Shield, Star, Zap, ArrowLeft
} from "lucide-react";
import { Button } from "@/components/ui/button";
import { Progress } from "@/components/ui/progress";
import { Switch } from "@/components/ui/switch";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { authAPI, lockedAppsAPI } from "@/lib/api";
...
const Account = () => {
  ...
  // ── Load user + locked apps on mount ─────────────────────────────────────
  useEffect(() => {
    const load = async () => {
      try {
        const [meData, appsData] = await Promise.all([
          authAPI.getMe(),
          lockedAppsAPI.checkStatus(),
        ]);

        const u = meData.user;
        setUser(u);

        // Sync preference toggles from the real user record
        setNotificationsEnabled(u.preferences?.notificationsEnabled ?? true);
        setAppLockEnabled(u.preferences?.appLockEnabled ?? false);

        // Map backend app shape to what the UI needs
        setLockedApps(
          appsData.apps.map((a: any) => ({
            _id:        a._id,
            name:       a.name,
            icon:       a.icon,
            locked:     a.isLocked,
            unlockTime: a.isLocked
              ? a.unlockCondition
              : a.unlockedUntil
                ? `Unlocked until ${new Date(a.unlockedUntil).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`
                : "Unlocked",
          }))
        );
      } catch (err) {
  ...
  const handleNotifications = async (val: boolean) => {
    ...
    try {
      await authAPI.updatePreferences({ notificationsEnabled: val });
    } catch (err) {
      console.error("Failed to update notifications preference:", err);
      setNotificationsEnabled(!val); // revert on failure
    }
  };
  ...
  const handleAppLock = async (val: boolean) => {
    ...
    try {
      await authAPI.updatePreferences({ appLockEnabled: val });
    } catch (err) {
      console.error("Failed to update app lock preference:", err);
      setAppLockEnabled(!val); // revert on failure
    }
  };
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center space-y-4">
          <p className="text-muted-foreground">Could not load account. Please try again.</p>
  ...
  const currentPlan = planConfig[user.plan as keyof typeof planConfig] ?? planConfig.free;
            ...
            <div className="flex-1">
              ...
              <div className="mt-2 flex items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground">
                  Member for {user.daysJoined} days
                </span>
              </div>
            </div>
          </div>
              ...
              {lockedApps.map((app) => (
                ...
                >
                  ...
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                    app.locked ? "bg-muted text-muted-foreground" : "bg-success/20 text-success"
                  }`}>
        ...
        <div className="account-card overflow-hidden animate-fade-in" style={{ animationDelay: "0.3s" }}>
          ...
          <div className="divide-y divide-border/50">
            ...
            <div className="account-setting-item">
              ...
              <Switch checked={notificationsEnabled} onCheckedChange={handleNotifications} />
            </div>
            ...
            <div className="account-setting-item">
              ...
              <Switch checked={appLockEnabled} onCheckedChange={handleAppLock} />
            </div>
